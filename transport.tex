...
\node [left] at (6,0) {$x_h$};
\node [above] at (4,2) {A};
\node [above] at (4,-2) {$T_0$};
\node [below] at (4,-2) {$x_0$};
\node at (3,-1) {B};
\node at (5,-1) {B};
\end{tikzpicture}
\end{figure}
...
\draw [gray,ultra thick] (2,0) arc [radius=2, start angle=180, end angle= 265];
\node [right] at (-1.7,0) {$\vec{J} (\Pi_A -\Pi_B) \Rightarrow$};
\node [left] at (9.7,0) {$\Rightarrow \vec{J} (\Pi_A -\Pi_B)$};
\node [above] at (4,2) {$\Pi_A \vec{J}$};
\node at (3,-1) {$\Pi_B \vec{J}$};
\node at (5,-1) {$\Pi_B \vec{J}$};
